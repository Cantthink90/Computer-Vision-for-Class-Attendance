...
class_names  = VGG2Dataset.classes  # list of folder names = identity labels
...
def make_svd_linear(layer, rank_ratio=0.5):
    ...
    #get rank
    rank = max(8, round(S.numel() * rank_ratio))
...
if __name__ == "__main__":
    ...
    model_original = AlexNet(num_classes=num_classes)
    ...
    results = []
    results.append(benchmark(model_original, testLoader, "AlexNet Original"))
    results.append(benchmark(model_SVD, testLoader, "AlexNet SVD"))
    results.append(benchmark(model_ft, testLoader, "AlexNet SVD Tuned"))
