...
if __name__ == "__main__":
    ...
    #load data of training and testing datasets
    ...
    # start up gpu to get accurate results later on
    ...
    #apply SVD and save
    print(f"\n=== SVD Layer Replacement (rank_ratio = {RANK_RATIO}) ===")
    replace_layers_with_svd(model, RANK_RATIO)
    ...
    for epoch in range(finetune_epochs):
        ...
        print(f"Fine-tune Epoch {epoch:>2} | Loss: {avg_loss:.4f} | "
              f"LR: {scheduler.get_last_lr()[0]:.6f} | Time: {time.time() - t0:.1f}s")

    evaluate(model, testLoader, "SVD — After Fine-tune")

    # save finetuned model
    torch.save(model.state_dict(), "lenet_svd_finetuned.pth")
    print("Saved → lenet_svd_finetuned.pth")
